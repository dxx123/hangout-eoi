...
public class Grok extends BaseFilter {
		...
		if (path.isDirectory()) {
			...
			try {
				...
				while ((sCurrentLine = br.readLine()) != null) {
					...
					this.patterns.put(sCurrentLine.split("\\s", 2)[0], sCurrentLine.split("\\s", 2)[1]);
				}
	...
	protected void prepare() {
		...
		if (jarFile.isFile()) { // Run with JAR file
			...
			try {
				load_patterns(new File(ClassLoader.getSystemResource("patterns").getFile()));
			} catch (Exception e) {
				logger.warn(e);
			}
		}
		...
		if (this.config.containsKey("pattern_paths")) {
			try {
				ArrayList<String> pattern_paths = (ArrayList<String>) this.config.get("pattern_paths");
		...
		for (String matchString : (ArrayList<String>) this.config.get("match")) {
			...
			Regex regex = null;
			try {
				regex = new Regex(matchString.getBytes("UTF8"), 0, matchString.getBytes("UTF8").length, Option.NONE,
			...
			try {
				...
				if (result != -1) {
					...
					for (Iterator<NameEntry> entry = regex.namedBackrefIterator(); entry.hasNext();) {
						...
						if (begin != -1) {
							event.put(new String(e.name, e.nameP, e.nameEnd - e.nameP),
									new String(bs, begin, end - begin, this.encoding));
						}
