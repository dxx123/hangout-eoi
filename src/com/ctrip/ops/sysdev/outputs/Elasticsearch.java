...
public class Elasticsearch extends BaseOutput {
	...
	protected void prepare() {
		...
		if (config.containsKey("document_id")) {
			try {
				this.idRender = new FreeMarkerRender((String) config.get("document_id"),
		...
		if (config.containsKey("index_type")) {
			try {
				this.indexTypeRender = new FreeMarkerRender((String) config.get("index_type"),
	...
	private void initESClient() throws NumberFormatException, UnknownHostException {
		...
		Settings settings = Settings.settingsBuilder().put("client.transport.sniff", sniff)
		...
		for (String host : hosts) {
			...
			esclient.addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName(h), Integer.parseInt(p)));
		}
		...
		bulkProcessor = BulkProcessor.builder(esclient, new BulkProcessor.Listener() {
			...
			public void beforeBulk(long arg0, BulkRequest arg1) {
				...
				.setFlushInterval(TimeValue.timeValueSeconds(flushInterval)).setConcurrentRequests(concurrentRequests)
				.build();
	}
